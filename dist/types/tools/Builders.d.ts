...
export declare class Builders {
     ...
     */
    static equalsOther<T>(target: T, prop?: types.KeyOf<T>): fns.Handler<T, boolean>;
    /**
     * 构建判断两个对象是否相等的函数, 其中返回函数的参数是一个迭代元素对象
     * @param target 匹配源对象
     * @param [predictor=(el=>el===target)] 断言属性或断言函数
     */
    static equalsOtherElement<T>(target: T, predictor?: fns.ArrayPredictor<T>): fns.Handler<types.IteratorItem<T>, boolean>;
    /**
     * 对象映射键
     ...
     */
    static toArrayKeyMapperHandler<T, R = any>(akm?: fns.ArrayKeyMapper<T, R>, mod?: 'throw' | 'element'): fns.ArrayKeyMapperHandler<T, R>;
    /**
     * 构建具有固定参数的调用函数
     * @param fn 函数引用
     * @param [args] 参数
     */
    static runner<T>(fn: fns.Function, ...args: any[]): T;
}
