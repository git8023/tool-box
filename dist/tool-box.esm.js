...
class ConsoleLogger {
    ...
    enableTrace = false;
    cacheStore = [];
    trace = (...args) => this.log(LogLevel.TRACE, Array.from(args), true);
    debug = (...args) => this.log(LogLevel.DEBUG, Array.from(args));
    info = (...args) => this.log(LogLevel.INFO, Array.from(args));
    warn = (...args) => this.log(LogLevel.WARN, Array.from(args));
    error = (...args) => this.log(LogLevel.ERROR, Array.from(args));
    fatal = (...args) => this.log(LogLevel.FATAL, Array.from(args));
    constructor() {
...
class Functions {
     ...
     * @param vog 数据或数据获取函数
     */
    static execOrGetter(vog) {
        return vog instanceof Function ? Functions.call(vog) : vog;
    }
    /**
     * 执行异步获取函数
     * @param oag 数据或同步/异步数据获取函数
     */
    static execOrAsyncGetter(oag) {
        if (oag instanceof Promise)
            return oag;
        const data = this.execOrGetter(oag);
...
class Events {
    ...
    static debouncer(duration = 100, debounceProp) {
        ...
        return createDecorator((options, fnKey) => {
    ...
    static log(params = true, returns = true) {
        return createDecorator((vm, fnKey) => {
...
export { Arrays, AsyncArrayStream, BError, Broadcast, Builders, Cast, Condition, ConsoleLogger, DataPool, DataPoolKey, Dates, Delete, Documents, Events, Functions, Get, Jsons, LogLevel, Logics, Logs, Objects, Observer, Post, Promises, PropChains, Put, Storages, StoreTools, Strings, Switcher, Validation };
